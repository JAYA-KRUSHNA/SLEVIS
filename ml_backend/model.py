...
class EnsemblePredictor:
    """
    Weighted Ensemble of ResidualDNN + AttentionLSTM.

    Combines predictions with learned weights for best accuracy.
    Default weights: DNN 0.6, LSTM 0.4 (DNN tends to be more stable).
    """

    def __init__(self, dnn_path=None, lstm_path=None, scaler_path=None, weights_path=None):
        ...
        if weights_path and os.path.exists(weights_path):
            with open(weights_path, 'r') as f:
                w = json.load(f)
                self.dnn_weight = w.get('dnn_weight', 0.6)
                self.lstm_weight = w.get('lstm_weight', 0.4)
            print(f"  ✅ Ensemble weights loaded: DNN={self.dnn_weight:.2f}, LSTM={self.lstm_weight:.2f}")
    ...
    def predict(self, vehicle_type, location, time_str, day):
        ...
        # Weighted average
        combined = self.dnn_weight * dnn_probs + self.lstm_weight * lstm_probs

        temp = ResidualDNN.__new__(ResidualDNN)
        result = temp._format_result(combined, vehicle_type, location, time_str, day, 'Ensemble(DNN+LSTM)')
        return result
    ...
    def optimize_weights(self, X_val, y_val):
        ...
        for w in np.arange(0.3, 0.8, 0.05):
            combined = w * dnn_preds + (1 - w) * lstm_preds
            try:
        ...
        self.dnn_weight = round(best_w, 2)
        self.lstm_weight = round(1 - best_w, 2)
        print(f"   Optimal: DNN={self.dnn_weight}, LSTM={self.lstm_weight} (AUC={best_auc:.4f})")

        return {'dnn_weight': self.dnn_weight, 'lstm_weight': self.lstm_weight, 'val_auc': round(best_auc, 4)}
